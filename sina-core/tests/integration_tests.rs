...
fn test_drawing_shapes() {
    ...
    // Test rectangle
    surface.canvas().draw_rect(Rect::new(10.0, 10.0, 50.0, 50.0), &paint);
    
    // Test circle
    surface.canvas().draw_circle(Point::new(50.0, 50.0), 20.0, &paint);
...
fn test_paint_configuration() {
    ...
    paint.set_stroke(StrokeStyle { width: 5.0, ..Default::default() });
