...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let samples = vec![
        ...
        (
            "Spanish",
            "¡Hola, Mundo! El rápido zorro marrón salta.",
            "DejaVuSans.ttf",
        ),
        (
            "French",
            "Bonjour, Monde! Le renard brun rapide saute.",
            "DejaVuSans.ttf",
        ),
        (
            "German",
            "Hallo, Welt! Der schnelle braune Fuchs springt.",
            "DejaVuSans.ttf",
        ),
        (
            "Portuguese",
            "Olá, Mundo! A rápida raposa marrom pula.",
            "DejaVuSans.ttf",
        ),
        ...
        (
            "Bangla",
            "হ্যালো বিশ্ব! দ্রুত বাদামী শিয়াল লাফ দেয়।",
    ...
    for (i, (lang, text, font_name)) in samples.iter().enumerate() {
        ...
        if let Some(path) = font_path {
            ...
            if let Some(font) = font {
                ...
                surface
                    .canvas()
                    .draw_text(text, Point::new(200.0, y), &font, 22.0, &text_paint);
    ...
    // Save output
    // Save output
    let path = "examples/output/text/multilingual.png";
